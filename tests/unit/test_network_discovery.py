...
class TestGetLocalSubnets:
    ...
    def test_fallback_when_no_interfaces(self):
        ...
        with patch("src.logic.network_discovery.socket.socket") as mock_cls:
            ...
            with patch("src.logic.network_discovery.socket.gethostname", side_effect=OSError):
                with patch(
                    "src.logic.network_discovery.socket.getaddrinfo", side_effect=OSError
                ):
                    result = get_local_subnets()
...
class TestDiscoverInstruments:
    ...
    def test_scpi_scan_finds_instruments(self):
        ...
        with patch("src.logic.network_discovery.get_local_subnets", return_value=["192.168.1"]):
            with patch(
                "src.logic.network_discovery._scan_subnet", return_value=instruments
            ):
    ...
    def test_progress_callback_called(self):
        ...
        callback = MagicMock()
        with patch("src.logic.network_discovery.get_local_subnets", return_value=["192.168.1"]):
            with patch("src.logic.network_discovery._scan_subnet", return_value=[]):
