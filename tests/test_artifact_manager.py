...
class TestArtifactManagerCollect:
    ...
    def test_collect_artifacts_returns_list(self, temp_artifact_dir):
        ...
        artifacts = manager.collect_artifacts(relative_path="calibration_vu123")
    ...
    def test_collect_artifacts_empty_for_missing_dir(self, tmp_path):
        ...
        artifacts = manager.collect_artifacts(relative_path="calibration_vu999")
    ...
    def test_collect_artifacts_excludes_non_png_files(self, tmp_path):
        ...
        manager = ArtifactManager(base_dir=str(tmp_path))
        artifacts = manager.collect_artifacts(relative_path="calibration_vu456")
...
class TestArtifactManagerPaths:
    ...
    def test_get_artifact_dir_format(self, tmp_path):
        ...
        manager = ArtifactManager(base_dir=str(tmp_path))
        path = manager.get_artifact_dir(relative_path="calibration_vu42")

        assert "calibration_vu42" in path

    def test_get_artifact_dir_different_paths(self, tmp_path):
        """get_artifact_dir should produce different paths for different inputs."""
        manager = ArtifactManager(base_dir=str(tmp_path))
        path1 = manager.get_artifact_dir(relative_path="calibration_vu100")
        path2 = manager.get_artifact_dir(relative_path="calibration_vu200")
